...
if( $op == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        $data = array( "uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "days" => intval($_GPC["days"]), "price" => intval($_GPC["price"]), "day_free_limit" => intval($_GPC["day_free_limit"]), "delivery_fee_free_limit" => (0 < intval($_GPC["delivery_fee_free_limit"]) ? intval($_GPC["delivery_fee_free_limit"]) : 0), "displayorder" => intval($_GPC["displayorder"]) );
        if( empty($data["title"]) ) 
        {
