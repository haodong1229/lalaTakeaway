define(['tiny', 'map', 'laytpl', 'jquery.range'], function(tiny, AMap, laytpl) {
	...
	trade.init = function(params) {
		...
		$(document).on('click', '#save-address', function(){
			...
			$.post(tiny.getUrl('errander/address/serve_address'), address, function(data){
				...
				if(result.message.errno != -1) {
					...
					var address_html = address.name;
					if(address.number) {
						address_html = address.name + '~' + address.number;
					}
					$('#start-address .item-title').removeClass('color-gray').html(address_html);
		...
		$(document).on('click', '.popup-select-buy-address .available-address-item', function(){
			...
			var address_html = $this.data('address');
			if($this.data('number')) {
				address_html = $this.data('address') + '~' + $this.data('number');
			}
			$('#start-address .item-title').removeClass('color-gray').html(address_html);
			order.start_address = {
				id: start_address_id,
