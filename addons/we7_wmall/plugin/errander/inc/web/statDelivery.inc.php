...
if( $op == "index" ) 
{
	...
	$condition_normal = (string) $condition . " and (delivery_success_time - delivery_assign_time < " . $config_takeout["delivery_timeout_limit"] . " * 60)";
	$condition_timeout = (string) $condition . " and (delivery_success_time - delivery_assign_time > " . $config_takeout["delivery_timeout_limit"] . " * 60)";
	$condition_before = (string) $condition . " and (delivery_success_time - delivery_assign_time <= " . $config_takeout["delivery_before_limit"] . " * 60)";
	if( $_W["isajax"] ) 
	{
		$stat = array( );
		...
		$chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_normal_order", "total_timeout_order", "total_before_order", "avg_normal_delivery_time" ), "titles" => array( "总配送", "正常送达订单", "超时订单", "提前订单", "普通单平均配送时长" ) );
		$i = $starttime;
		while( $i <= $endtime ) 
		{
			$chart["days"][] = $i;
