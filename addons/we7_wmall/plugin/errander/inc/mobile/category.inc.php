...
if( $op == "index" ) 
{
	...
	$title = (string) $types[$category["type"]]["text"] . " - " . $category["title"];
	...
	if( !empty($data) ) 
	{
		...
		$predict_time = (string) $delivery_time["times"][$predict_index]["start"] . "~" . $delivery_time["times"][$predict_index]["end"];
		$text_time = "立即配送";
	...
	{
		$price_select = array( array( "id" => 1, "title" => "100元以下" ), array( "id" => 2, "title" => "100元-200元" ), array( "id" => 3, "title" => "200元-300元" ), array( "id" => 4, "title" => "300元-400元" ), array( "id" => 5, "title" => "400元-500元" ) );
	}
