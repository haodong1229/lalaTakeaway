	...
	else 
	{
		...
		else 
		{
			if( $type == "cancel" ) 
			{
				...
				{
					pdo_update("tiny_wmall_activity_redpacket_record", array( "status" => 1, "usetime" => 0, "order_id" => 0 ), array( "uniacid" => $_W["uniacid"], "uid" => $_W["member"]["uid"], "order_id" => $order["id"], "scene" => "paotui" ));
				}
...
function errander_order_calculate_delivery_fee($idOrCategory, $extra, $is_calculate = 0) 
{
	...
	$deliveryinfo = array( );
	for( $i = 0; $i <= intval($rule_fee["fee_day_limit"]);
	$i++ ) 
	{
