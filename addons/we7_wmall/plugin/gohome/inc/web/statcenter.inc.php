...
if( $op == "list" ) 
{
	...
	if( $_W["isajax"] ) 
	{
		...
		$chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_fee", "final_fee", "plateform_serve_fee", "refund_fee" ), "titles" => array( "有效订单量", "营业总额", "总入账", "佣金收入", "总退款" ) );
		$i = $starttime;
		while( $i <= $endtime ) 
		{
			$chart["days"][] = $i;
		...
		$records = pdo_fetchall("SELECT stat_day, count(*) as total_success_order,round(sum(final_fee), 2) as final_fee, round(sum(price), 2) as total_fee, round(sum(plateform_serve_fee), 2) as plateform_serve_fee\r\n\t\tFROM " . tablename("tiny_wmall_gohome_order") . $condition . " and (status = 6 or status = 5) group by stat_day", $params);
		if( !empty($records) ) 
		{
	...
	$records_temp = pdo_fetchall("SELECT stat_day, count(*) as total_success_order, round(sum(final_fee), 2) as final_fee, round(sum(price), 2) as total_fee, round(sum(plateform_serve_fee), 2) as plateform_serve_fee\r\n\t FROM " . tablename("tiny_wmall_gohome_order") . $condition . " and (status = 6 or status = 5) group by stat_day", $params, "stat_day");
	...
	while( $starttime <= $i ) 
	{
		...
		{
			$records[] = array( "stat_day" => $i, "total_success_order" => 0, "total_fee" => 0, "final_fee" => 0, "plateform_serve_fee" => 0 );
		}
		else 
		{
