...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$data = array( "uniacid" => $_W["uniacid"], "title" => $title, "thumb" => trim($_GPC["thumb"]), "displayorder" => intval($_GPC["displayorder"]), "type" => "gohome", "status" => intval($_GPC["status"]), "wxapp_link" => trim($_GPC["wxapp_link"]) );
		if( !empty($id) ) 
		{
		...
		imessage(error(0, "编辑幻灯片成功"), iurl("gohome/slide/list"), "ajax");
	}
...
if( $op == "list" ) 
{
	...
	if( checksubmit() ) 
	{
		...
		imessage("编辑幻灯片成功", iurl("gohome/slide/list"), "success");
	}
	$condition = " where uniacid = :uniacid and type = :type";
	$params = array( ":uniacid" => $_W["uniacid"], ":type" => "gohome" );
...
if( $op == "slideagent" ) 
{
	...
	if( $_W["ispost"] && $_GPC["set"] == 1 ) 
	{
		...
		imessage(error(0, "批量操作修改成功"), iurl("gohome/slide/list"), "ajax");
	}
