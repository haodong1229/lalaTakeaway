...
if ($op == 'store') {
	...
	if (!(empty($store))) {
		...
		if ($store['haodian_status'] == 1) {
		 ...
		 else if ($store['haodian_status'] == 6) {
	...
	if ($_W['ispost']) {
		...
		$data = array('uniacid' => $_W['uniacid'], 'agentid' => ($clerk['agentid'] ? $clerk['agentid'] : $_W['agentid']), 'title' => $title, 'logo' => trim($GPC_store['logo']), 'address' => trim($GPC_store['address']), 'telephone' => trim($GPC_store['telephone']), 'content' => trim($GPC_store['content']), 'status' => $config_store['settle']['audit_status'], 'business_hours' => iserializer(array(
	...
	$result = array(
		...
		'config'    => array('qualification_verify_status' => intval($config_store['settle']['qualification_verify_status']), 'meal' => ($config_haodian['settle']['status'] == 1 ? $config_haodian['settle']['meal'] : array()))
		);
