...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$data = array( "uniacid" => $_W["uniacid"], "title" => $title, "thumb" => trim($_GPC["thumb"]), "displayorder" => intval($_GPC["displayorder"]), "type" => "haodian", "status" => intval($_GPC["status"]), "wxapp_link" => trim($_GPC["wxapp_link"]) );
		if( !empty($slide) ) 
		{
...
else 
{
	if( $op == "list" ) 
	{
		...
		$params = array( ":uniacid" => $_W["uniacid"], ":type" => "haodian" );
			...
			else 
			{
				if( $op == "slideagent" ) 
				{
					...
					if( $_W["ispost"] && $_GPC["set"] == 1 ) 
					{
						...
						imessage(error(0, "批量操作修改成功"), iurl("haodian/slide/list"), "ajax");
					}
