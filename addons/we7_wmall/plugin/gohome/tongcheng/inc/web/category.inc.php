...
else 
{
	if( $op == "post" ) 
	{
		...
		if( $_W["ispost"] ) 
		{
			$data = array( "uniacid" => intval($_W["uniacid"]), "agentid" => intval($_W["agentid"]), "displayorder" => intval($_GPC["displayorder"]), "title" => trim($_GPC["title"]), "content" => trim($_GPC["content"]), "thumb" => trim($_GPC["thumb"]), "price" => floatval($_GPC["price"]), "status" => intval($_GPC["status"]), "is_hot" => intval($_GPC["is_hot"]), "link" => trim($_GPC["link"]), "tags" => explode("\n", trim($_GPC["tags"])) );
