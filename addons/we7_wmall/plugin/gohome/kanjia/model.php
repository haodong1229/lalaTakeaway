...
{
    global $_W;
    $category = pdo_get("tiny_wmall_kanjia_category", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
    if( !empty($category) ) 
    {
...
function kanjia_get_activitylist($filter = array(  ))
{
    ...
    if( !empty($activitys) ) 
    {
        ...
        foreach( $activitys as &$val ) 
        {
            ...
            $val["sailed_percent"] = round($val["sailed"] / ($val["sailed"] + $val["total"]) * 100, 2);
...
function kanjia_get_helper($userid, $orderby = "")
{
    ...
    $condition = " where a.uniacid = :uniacid and a.agentid = :agentid and userid = :userid ";
    $params = array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":userid" => $userid );
...
function kanjia_bargain_check($activityid, $uid)
{
    ...
    $times = pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_kanjia_helprecord") . " where uniacid = :uniacid and agentid = :agentid and activityid = :activityid and userid = :userid and authorid = :authorid and uid = :uid", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":activityid" => $activityid, ":userid" => $takeinfo["id"], ":authorid" => $takeinfo["uid"], ":uid" => $_W["member"]["uid"] ));
    if( 0 < $times ) 
    {
    ...
    {
        $records = pdo_fetchall("select authorid from " . tablename("tiny_wmall_kanjia_helprecord") . " where uniacid = :uniacid and agentid = :agentid and activityid = :activityid and uid = :uid group by authorid ", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":activityid" => $activityid, ":uid" => $_W["member"]["uid"] ));
    ...
    if( 0 < $activity["dayhelplimit"] ) 
    {
        ...
        $daytimes = pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_kanjia_helprecord") . " where uniacid = :uniacid and agentid = :agentid and activityid = :activityid and userid = :userid and authorid = :authorid and createtime >= " . $starttime . " and createtime < " . $endtime, array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":activityid" => $activityid, ":userid" => $takeinfo["id"], ":authorid" => $takeinfo["uid"] ));
...
{
    global $_W;
    $condition = " where a.uniacid = :uniacid and a.agentid = :agentid and a.activityid = :activityid";
    $params = array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":activityid" => intval($filter["activityid"]) );
    $sid = intval($filter["sid"]);
    if( 0 < $sid ) 
    {
...
function kanjia_check_activity_order_num($activityOrId)
{
    ...
    $totaljoin = pdo_fetchcolumn("select count(*) from " . tablename("tiny_wmall_gohome_order") . " where uniacid = :uniacid and agentid = :agentid and goods_id = :goods_id and is_pay = 1 and status > 1 and status < 7", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":goods_id" => $activity["id"] ));
