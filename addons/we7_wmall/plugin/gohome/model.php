...
function gohome_get_menu($type = "")
{
    ...
    if( 0 && $type == "gohome" ) 
    {
        ...
        if( !empty($config_menu["gohome"]) ) 
        {
            $menu = pdo_get("tiny_wmall_diypage_menu", array( "uniacid" => $_W["uniacid"], "id" => $config_menu["gohome"] ), array( "data" ));
            return json_decode(base64_decode($menu["data"]), true);
        }
