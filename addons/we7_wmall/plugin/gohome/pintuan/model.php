	...
	if( !empty($goods) ) 
	{
		...
		foreach( $goods as &$da ) 
		{
			...
			if( $da["total"] != -1 ) 
			{
				...
				$da["sailed_percent"] = ($orgin_total < $da["sailed"] + $da["falesailed"] ? round($da["sailed"] / $orgin_total * 100, 1) : round(($da["sailed"] + $da["falesailed"]) / $orgin_total * 100, 1));
			}
		}
	}
	return $goods;
...
{
	global $_W;
	$condition = " where a.uniacid = :uniacid and a.agentid = :agentid and a.order_type = :order_type";
	$params = array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":order_type" => "pintuan" );
