...
if( $op == "index" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$da = array( );
		$categorys = pdo_fetchall("select id, title, menu_id from " . tablename("tiny_wmall_cloudgoods_goods_category") . " where uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"] ));
		$categorys_title = array( );
		if( !empty($categorys) ) 
		{
			foreach( $categorys as $val ) 
			{
				$categorys_title[] = trim($val["title"]);
			}
		}
		foreach( $data as $da ) 
		{
