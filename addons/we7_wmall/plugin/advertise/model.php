...
function get_advertise_info($type)
{
    ...
    {
        $sailed = pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_advertise_trade") . "where uniacid = :uniacid and type = :type and status = 1", array( ":uniacid" => $_W["uniacid"], ":type" => $type ));
    ...
    else
    {
        if( $type == "stick" ) 
        {
            $sailed = pdo_fetchall("select * from" . tablename("tiny_wmall_advertise_trade") . "where uniacid = :uniacid and type = :type and status = 1", array( ":uniacid" => $_W["uniacid"], ":type" => "stick" ), "displayorder");
