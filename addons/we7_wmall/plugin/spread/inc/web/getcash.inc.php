...
if( $op == "transfers" ) 
{
    ...
    $params = array( "partner_trade_no" => $log["trade_no"], "openid" => $log["account"]["openid"], "check_name" => "FORCE_CHECK", "re_user_name" => $log["account"]["realname"], "amount" => $log["final_fee"] * 100, "desc" => (string) $log["account"]["realname"] . date("Y-m-d H:i", $log["addtime"]) . "推广佣金提现申请" );
    mload()->classs("wxpay");
    if( $log["channel_from"] == "wxapp" ) 
    {
        ...
        {
            $member = pdo_get("tiny_wmall_members", array( "uniacid" => $_W["uniacid"], "uid" => $log["spreadid"] ), array( "openid_wxapp" ));
            $params["openid"] = $member["openid_wxapp"];
        }
