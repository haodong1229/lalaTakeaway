...
{
	$data = array( "wechat" => array( "css" => "label label-success", "text" => "微信" ), "credit" => array( "css" => "label label-warning", "text" => "账户余额" ) );
	if( $key == "all" ) 
	{
		return $data[$channel];
	}
...
function spread_order_balance($order_id, $type = "takeout") 
{
	global $_W;
	global $_GPC;
	$routers = array( "takeout" => array( "end_status" => array( 5 ), "order_type_cn" => "外卖单" ), "paotui" => array( "table" => "tiny_wmall_errander_order", "end_status" => array( 3 ), "order_type_cn" => "跑腿单" ), "gohome" => array( "table" => "tiny_wmall_gohome_order", "end_status" => array( 5, 6 ), "order_type_cn" => "生活圈订单" ) );
	$router = $routers[$type];
	if( $type == "takeout" ) 
	{
		$order = pdo_fetch("select a.id,a.uid,a.spread1,a.spread2,a.spreadbalance,a.data,a.endtime,a.status,b.oid from " . tablename("tiny_wmall_order") . " as a left join " . tablename("tiny_wmall_order_comment") . " as b on a.id = b.oid where a.uniacid = :uniacid and a.id = :id", array( ":uniacid" => $_W["uniacid"], ":id" => $order_id ));
	}
	else 
	{
		$order = pdo_get($router["table"], array( "uniacid" => $_W["uniacid"], "id" => $order_id ), array( "id", "uid", "spread1", "spread2", "spreadbalance", "data", "status" ));
	}
	if( empty($order) ) 
	{
	...
	if( $balance == 1 ) 
	{
		...
		pdo_update($router["table"], array( "spreadbalance" => 1 ), array( "id" => $order["id"] ));
	}
	return true;
}
