...
else 
{
	if( $op == "post" ) 
	{
		...
		if( $_W["ispost"] ) 
		{
			$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => trim($_GPC["title"]), "thumb" => trim($_GPC["thumb"]), "link" => trim($_GPC["link"]), "status" => trim(trim($_GPC["status"])), "displayorder" => intval($_GPC["displayorder"]) );
			if( empty($id) ) 
			{
