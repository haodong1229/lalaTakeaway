...
if( $op == "list" ) 
{
    ...
    if( $_W["ispost"] && !empty($_GPC["ids"]) ) 
    {
        foreach( $_GPC["ids"] as $k => $v ) 
        {
            ...
            pdo_update("tiny_wmall_kanjia", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => intval($v) ));
        }
    ...
    $categorys = pdo_fetchall("select id,title from " . tablename("tiny_wmall_kanjia_category") . " where uniacid = :uniacid and agentid = :agentid", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"] ), "id");
