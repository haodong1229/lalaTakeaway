...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => trim($_GPC["title"]), "thumb" => trim($_GPC["thumb"]), "description" => trim($_GPC["description"]), "content" => htmlspecialchars_decode($_GPC["content"]), "displayorder" => intval($_GPC["displayorder"]), "status" => intval($_GPC["status"]), "addtime" => TIMESTAMP, "wxapp_link" => trim($_GPC["wxapp_link"]) );
		if( !empty($id) ) 
		{
