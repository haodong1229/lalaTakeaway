...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => $title, "thumb" => trim($_GPC["thumb"]), "displayorder" => intval($_GPC["displayorder"]), "type" => "gohome", "status" => intval($_GPC["status"]), "wxapp_link" => trim($_GPC["wxapp_link"]) );
		if( !empty($id) ) 
		{
...
if( $op == "list" ) 
{
	...
	$params = array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":type" => "gohome" );
