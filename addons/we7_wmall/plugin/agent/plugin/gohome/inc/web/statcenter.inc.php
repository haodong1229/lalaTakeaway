...
if( $op == "list" ) 
{
	...
	if( $_W["isajax"] ) 
	{
		...
		$chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_fee", "final_fee", "plateform_serve_fee", "agent_serve_fee", "refund_fee" ), "titles" => array( "有效订单量", "营业总额", "总入账", "代理佣金收入", "平台服务费", "总退款" ) );
		$i = $starttime;
		while( $i <= $endtime ) 
		{
			$chart["days"][] = $i;
