...
if( $op == "index" ) 
{
	...
	if( $_W["isajax"] ) 
	{
		...
		$chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_fee", "delivery_fee", "deliveryer_fee", "refund_fee" ), "titles" => array( "有效订单量", "营业总额", "总入账", "配送费收入", "配送员配送费支出", "总退款" ) );
		$i = $starttime;
		while( $i <= $endtime ) 
		{
			$chart["days"][] = $i;
	...
	while( $starttime <= $i ) 
	{
		...
		{
			$records[] = array( "stat_day" => $i, "total_success_order" => 0, "total_fee" => 0, "deliveryer_fee" => 0, "delivery_fee" => 0, "refund_fee" => 0, "agent_serve_fee" => 0 );
		}
		else 
		{
