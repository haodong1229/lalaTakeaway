...
if( $op == "list" ) 
{
    ...
    if( !empty($reports) ) 
    {
        ...
        $stores = pdo_fetchall("select id,title from " . tablename("tiny_wmall_store") . " where uniacid = :uniacid and agentid = :agentid and id in (" . $stores_str . ")", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"] ), "id");
    }
...
if( $op == "status" ) 
{
    ...
    pdo_update("tiny_wmall_report", array( "status" => $status ), array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
