	...
	if( $_W["ispost"] ) 
	{
		if( !empty($_GPC["ids"]) ) 
		{
			foreach( $_GPC["ids"] as $k => $v ) 
			{
				...
				pdo_update("tiny_wmall_news", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => intval($v) ));
			}
		}
...
{
	$_W["page"]["title"] = "添加资讯";
	$categorys = pdo_fetchall("SELECT * FROM " . tablename("tiny_wmall_news_category") . " WHERE uniacid = :uniacid AND agentid = :agentid ORDER BY id DESC", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"] ));
	$id = intval($_GPC["id"]);
	if( 0 < $id ) 
	{
		$item = pdo_get("tiny_wmall_news", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
	}
	if( $_W["ispost"] ) 
	{
		$_GPC["title"] = (trim($_GPC["title"]) ? trim($_GPC["title"]) : imessage(error(-1, "资讯名称不能为空"), "", "ajax"));
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => $_GPC["title"], "cateid" => intval($_GPC["cateid"]), "desc" => trim($_GPC["desc"]), "content" => htmlspecialchars_decode($_GPC["content"]), "thumb" => trim($_GPC["thumb"]), "author" => trim($_GPC["author"]), "click" => intval($_GPC["click"]), "is_display" => intval($_GPC["is_display"]), "is_show_home" => intval($_GPC["is_show_home"]), "displayorder" => intval($_GPC["displayorder"]), "addtime" => TIMESTAMP );
		if( !$id ) 
		{
		...
		{
			pdo_update("tiny_wmall_news", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
		}
...
{
	$id = intval($_GPC["id"]);
	pdo_delete("tiny_wmall_news", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
...
if( $op == "is_show_home" ) 
{
	...
	pdo_update("tiny_wmall_news", array( "is_show_home" => $is_show_home ), array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
...
if( $op == "is_display" ) 
{
	...
	pdo_update("tiny_wmall_news", array( "is_display" => $is_display ), array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
	imessage(error(0, ""), "", "ajax");
}
