...
if( $op == "list" ) 
{
	...
	$ads = pdo_fetchall("select * from " . tablename("tiny_wmall_slide") . " where uniacid = :uniacid and agentid = :agentid and type = :type order by displayorder desc", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":type" => 3 ));
}
if( $op == "post" ) 
{
	$_W["page"]["title"] = "编辑广告";
	...
	if( 0 < $id ) 
	{
		$ad = pdo_get("tiny_wmall_slide", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id, "type" => 3 ));
	}
	if( $_W["ispost"] ) 
	{
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => trim($_GPC["title"]), "thumb" => trim($_GPC["thumb"]), "link" => trim($_GPC["link"]), "displayorder" => intval($_GPC["displayorder"]), "status" => intval($_GPC["status"]), "type" => 3 );
		if( !empty($ad["id"]) ) 
		{
