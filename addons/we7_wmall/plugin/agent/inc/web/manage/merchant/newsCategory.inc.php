	...
	if( $_W["ispost"] ) 
	{
		if( !empty($_GPC["ids"]) ) 
		{
			foreach( $_GPC["ids"] as $k => $v ) 
			{
				...
				pdo_update("tiny_wmall_news_category", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => intval($v) ));
			}
		}
...
if( $op == "post" ) 
{
	...
	if( 0 < $id ) 
	{
		$item = pdo_get("tiny_wmall_news_category", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
	}
	if( $_W["ispost"] ) 
	{
		$_GPC["title"] = (trim($_GPC["title"]) ? trim($_GPC["title"]) : imessage(error(-1, "分类名称不能为空"), "", "ajax"));
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => $_GPC["title"], "displayorder" => intval($_GPC["displayorder"]) );
		if( !$id ) 
		{
		...
		{
			pdo_update("tiny_wmall_news_category", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
		}
...
{
	$id = intval($_GPC["id"]);
	pdo_delete("tiny_wmall_news_category", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
