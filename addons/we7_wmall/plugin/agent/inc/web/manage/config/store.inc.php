...
if( $op == "serve_fee" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$serve_fee = array( "fee_takeout" => $fee_takeout, "fee_selfDelivery" => $fee_selfDelivery, "fee_instore" => $fee_instore, "fee_paybill" => $fee_paybill, "store_label_new" => intval($_GPC["store_label_new"]) );
		...
		else 
		{
			...
			else 
			{
				...
				else 
				{
					...
					{
						$extra["delivery_extra"] = iserializer(array( "store_bear_deliveryprice" => floatval($_GPC["store_bear_deliveryprice"]) ));
