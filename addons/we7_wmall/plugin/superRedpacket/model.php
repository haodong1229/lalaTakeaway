				...
				if (($start_hour <= TIMESTAMP) && (TIMESTAMP < $end_hour)) 
				{
					...
					if ($can_grant == 1) 
					{
						foreach ($gift_redpacket['data']['redpackets'] as $redpacket ) 
						{
							...
							$params = array('title' => $redpacket['name'], 'activity_id' => $gift_redpacket['id'], 'uid' => $uid, 'openid' => $openid, 'channel' => 'superRedpacket', 'type' => 'gift', 'discount' => $redpacket['discount'], 'condition' => $redpacket['condition'], 'grant_days_effect' => $redpacket['grant_days_effect'], 'days_limit' => $redpacket['use_days_limit'], 'is_show' => 1, 'scene' => $redpacket['scene']);
							$times_limit = array();
							if (!(empty($redpacket['times']))) 
							{
