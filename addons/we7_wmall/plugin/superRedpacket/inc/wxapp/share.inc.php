...
if(!$_W['ispost']) {
	...
	$_W['_share'] = array(
		'title' => $activity['data']['share']['title'],
		'desc' => $activity['data']['share']['desc'],
		'imgUrl' => tomedia($activity['data']['share']['imgUrl']),
		'link' => ivurl('/pages/superRedpacket/index', array('order_id' => $order_id), true)
	);
}
...
if($op == 'index') {
	...
	$result = array(
		'activity_img' => $activity['data']['activity']['image'],
		'activity_title' => $activity['name'],
		'agreement' => nl2br($activity['data']['activity']['agreement']),
	);
...
if($op == 'grant') {
	...
	if(empty($is_exist) && $grant['packet_dosage'] > 0) {
