...
if( $op == "list" ) 
{
	...
	$params = array( ":uniacid" => $_W["uniacid"], ":type" => "share" );
...
if( $op == "post" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$insert = array( "uniacid" => $_W["uniacid"], "name" => $data["activity"]["name"], "type" => "share", "data" => base64_encode(json_encode($data)), "condition" => floatval($data["activity"]["condition"]), "starttime" => $startime, "endtime" => $endtime, "addtime" => TIMESTAMP, "status" => intval($data["activity"]["status"]) );
		if( 0 < $id ) 
		{
		...
		{
			$insert = array( "uniacid" => $_W["uniacid"], "activity_id" => $activity_id, "title" => trim($row["name"]), "discount" => floatval($row["discount"]), "condition" => floatval($row["condition"]), "grant_days_effect" => intval($row["grant_days_effect"]), "use_days_limit" => intval($row["use_days_limit"]), "nums" => intval($row["nums"]) );
			$times_limit = array( );
			if( !empty($row["times"]) ) 
			{
...
if( $op == "records" ) 
{
	...
	$params = array( ":uniacid" => $_W["uniacid"], ":activity_id" => $activity_id );
	$keyword = trim($_GPC["keyword"]);
	if( !empty($keyword) ) 
	{
