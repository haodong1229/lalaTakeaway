...
function storebd_user_order_commision($order)
{
	...
	if ($config_commision['type'] == 1) {
	 ...
	 else if ($config_commision['type'] == 2) {
...
{
	$data = array(
		1 => array('css' => 'label label-success', 'text' => '订单入账'),
...
function storebd_user_getcash_transfers($id)
{
	...
	if (empty($storebd_user) || empty($storebd_user['title']) || empty($storebd_user['openid'])) {
		return error(-1, '店铺推广员微信信息不完善,无法进行微信付款');
	}


	$params = array('partner_trade_no' => $log['trade_no'], 'openid' => (!(empty($log['account']['openid'])) ? $log['account']['openid'] : $storebd_user['openid']), 'check_name' => 'FORCE_CHECK', 're_user_name' => $storebd_user['title'], 'amount' => $log['final_fee'] * 100, 'desc' => $storebd_user['title'] . \date('Y-m-d H:i', $log['addtime']) . '店铺推广佣金提现申请');
	mload()->classs("wxpay");

	if ($log['channel'] == 'wxapp') {
		$params['openid'] = ((!(empty($log['account']['openid_wxapp'])) ? $log['account']['openid_wxapp'] : $storebd_user['openid_wxapp']));
