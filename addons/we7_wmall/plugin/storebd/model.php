...
function storebd_user_getcash_transfers($id) 
{
	...
	$params = array( "partner_trade_no" => $log["trade_no"], "openid" => $log["account"]["openid"], "check_name" => "FORCE_CHECK", "re_user_name" => $storebd_user["title"], "amount" => $log["final_fee"] * 100, "desc" => (string) $storebd_user["title"] . date("Y-m-d H:i", $log["addtime"]) . "店铺推广佣金提现申请" );
	mload()->classs("wxpay");
