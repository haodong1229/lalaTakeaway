    ...
    if( $op == "application" ) 
    {
        ...
        if( $_W["ispost"] ) 
        {
            ...
            if( MODULE_FAMILY == "wxapp" ) 
            {
                ...
                {
                    imessage(error(-1, "未获取到店铺推广员针对小程序的openid, 请完善账户信息"), "", "ajax");
                }
            ...
            $data = array( "uniacid" => $_W["uniacid"], "bd_id" => $storebd_user["id"], "trade_no" => date("YmdHis") . random(10, true), "get_fee" => $get_fee, "take_fee" => $take_fee, "final_fee" => $final_fee, "channel" => (MODULE_FAMILY == "wxapp" ? "wxapp" : "weixin"), "account" => iserializer(array( "realname" => $storebd_user["realname"], "openid" => $openid, "avatar" => $storebd_user["avatar"], "nickname" => $storebd_user["nickname"], "openid_wxapp" => $storebd_user["openid_wxapp"] )), "status" => 2, "addtime" => TIMESTAMP );
