...
if( $_W["ispost"] ) 
{
	...
	$ordergrant = array( "status" => intval($_GPC["status"]), "cycle" => intval($_GPC["cycle"]), "grantType" => trim($_GPC["grantType"]), "first_order_grant" => floatval($_GPC["first_order_grant"]), "days_order_grant" => floatval($_GPC["days_order_grant"]), "continuous" => $continuous, "special" => $special, "all" => $all, "agreement" => htmlspecialchars_decode($_GPC["agreement"]), "ordergrant_share" => $ordergrant_share );
