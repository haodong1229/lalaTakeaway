...
function mealRedpacket_exchanges_getall($id) 
{
	...
	$exchanges = pdo_fetchall('select a.*, b.title, b.score from ' . tablename('tiny_wmall_mealredpacket_exchange') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.redpacketid = :redpacketid limit ' . (($page - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':redpacketid' => $id));
	if (!(empty($exchanges))) 
	{
...
function mealRedpacket_order_update($orderOrId, $type, $extra = array()) 
{
	...
	if ($type == 'pay') 
	{
		...
		if (!(empty($meal_redpackets_data))) 
		{
			...
			{
				$data = array('title' => $redpacket['name'], 'channel' => 'mealRedpacket', 'type' => 'grant', 'discount' => $redpacket['discount'], 'days_limit' => $redpacket['use_days_limit'], 'grant_days_effect' => $redpacket['grant_days_effect'], 'condition' => $redpacket['condition'], 'uid' => $order['uid'], 'activity_id' => $order['sid']);
				$times_limit = array();
			...
			if (!(empty($openid))) 
			{
				...
				$params = array('first' => '您在' . $config['mall']['title'] . '的账户有' . $num . '个代金券到账', 'keyword1' => date('Y-m-d H:i', TIMESTAMP), 'keyword2' => '代金券到账', 'keyword3' => $discount . '元', 'remark' => implode("\n", array('感谢您对' . $config['mall']['title'] . '平台的支持与厚爱。点击查看红包>>')));
...
function mealRedpacket_plus_order_update($orderOrId, $type, $extra = array()) 
{
	...
	if ($type == 'pay') 
	{
		...
		if (!(empty($order['data']['meal']['data']))) 
		{
			...
			{
				$data = array('title' => $redpacket['name'], 'channel' => 'mealRedpacket_plus', 'type' => 'grant', 'discount' => $redpacket['discount'], 'days_limit' => $redpacket['use_days_limit'], 'grant_days_effect' => $redpacket['grant_days_effect'], 'condition' => $redpacket['condition'], 'uid' => $order['uid']);
				$times_limit = array();
				if (!(empty($redpacket['times']))) 
				{
					foreach ($redpacket['times'] as $time ) 
