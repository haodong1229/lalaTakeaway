...
if( $op == "index" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		$bargain = array( "status" => intval($_GPC["status"]), "is_home_display" => intval($_GPC["is_home_display"]), "home_number" => intval($_GPC["home_number"]), "template" => intval($_GPC["template"]), "thumb" => trim($_GPC["thumb"]), "agreement" => htmlspecialchars_decode($_GPC["agreement"]), "share" => $share );
