...
if( $op == "invite" ) 
{
    ...
    if( !empty($keyword) ) 
    {
        $condition .= " and (a.share_uid like '%" . $keyword . "%' or a.follow_uid like '%" . $keyword . "%' or b.realname like '%" . $keyword . "%' or b.nickname like '%" . $keyword . "%' or mobile like '%" . $keyword . "%')";
    }
    ...
    $invites = pdo_fetchall("select a.*,b.nickname as share_nickname,b.realname as share_realname,b.avatar as share_avatar from " . tablename("tiny_wmall_shareredpacket_invite_record") . " as a left join " . tablename("tiny_wmall_members") . " as b on a.share_uid = b.uid" . $condition . " order by id desc limit " . ($pindex - 1) * $psize . "," . $psize, $params);
    if( !empty($invites) ) 
    {
        foreach( $invites as &$row ) 
        {
            ...
            $row["follow_nickname"] = $follow_member["nickname"];
