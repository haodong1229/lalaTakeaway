define(['jquery.ui'], function(ui) {
	...
	diy.initParts = function(){
		...
		var partGroup = {
			...
			1: ['fixedsearch', 'searchbar', 'navs', 'notice', 'danmu', 'selective', 'bargain',  'waimai_goods', 'waimai_stores', 'waimai_allstores', 'graphic', 'listmenu', 'cart', 'redpacket', 'activity'],
	...
	diy.getParts = function(){
		diy.parts = {
			...
			activity: {
				...
				data: {
					C0123456789101: {
						imgurl: '../addons/we7_wmall/plugin/wxapp/static/img/default/store-3.jpg',
						linkurl: '',
						text: '活动一',
						color: '#ff2d4b',
						placeholder: '活动提示',
			...
			operation: {
				...
				data: {
					...
					C0123456789105: {
						imgurl: '../addons/we7_wmall/plugin/wxapp/static/img/default/navs-1.png',
						linkurl: 'pages/store/assign?sid='+diy.store_id,
						text: '排号',
						color: '#333333',
						decoration: '店内排号',
						dec_color: '#a0a0a0'
			...
			if(itemid == 'page') {
				...
				if(bindchild) {
					if(bindparent) {
						...
						if(!diy.items[itemid][bindchild]) {
							diy.items[itemid][bindchild] = {};
						}
		...
		$(".btn-save").unbind('click').click(function() {
			...
			irequire(['tiny'], function(tiny){
				if(diy.type == 3) {
					...
					$.post(tiny.getUrl('wxapp/diyPage/post'), {id: diy.id, data: diy.data}, function(ret) {
						var ret = ret.message;
						if(ret.errno != 0) {
							Notify.error(ret.message);
