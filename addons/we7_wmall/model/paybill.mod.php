...
function paybill_order_status_notice($orderOrid, $type, $extra = array(  ))
{
    ...
    if( !empty($order["openid"]) ) 
    {
        ...
        {
            $title = "您的订单已付款";
            $remark = array( "门店名称: " . $store["title"], "支付方式: " . $order["pay_type_cn"], "支付时间: " . date("m-d H:i:s", $order["paytime"]), "订单金额: ￥" . $order["total_fee"], "优惠金额: ￥" . $order["discount_fee"], "实付金额: ￥" . $order["final_fee"] );
        }
...
function paybill_order_clerk_notice($orderOrid, $type, $extra = array(  ))
{
    ...
    if( $type == "pay" ) 
    {
        ...
        $remark = array( "支付方式: " . $order["pay_type_cn"], "支付时间: " . date("m-d H:i:s", $order["paytime"]), "订单金额: ￥" . $order["total_fee"], "优惠金额: ￥" . $order["discount_fee"], "实付金额: ￥" . $order["final_fee"] );
    }
...
function paybill_order_update_bill($orderOrid)
{
    ...
    if( $account["fee_paybill"]["type"] == 1 ) 
    {
        ...
        $text = (string) $order["total_fee"] . " x " . $fee_rate . "%";
        $plateform_serve = array( "type" => 1, "fee_rate" => $fee_rate, "fee" => $plateform_serve_fee, "note" => $text );
    }
    else
    {
        $plateform_serve_fee = $account["fee_paybill"]["fee"];
        $plateform_serve = array( "type" => 2, "fee_rate" => 0, "fee" => $account["fee_paybill"]["fee"], "note" => "每单固定" . $account["fee_paybill"]["fee"] . "元" );
    }
    ...
    if( $_W["is_agent"] ) 
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $text = (string) $order["total_fee"] . " x " . $agent_serve_rate . "%";
    ...
    $date = array( "agent_final_fee" => $agent_final_fee, "agent_serve" => iserializer($agent_serve), "agent_serve_fee" => $agent_serve_fee, "plateform_serve" => iserializer($plateform_serve), "plateform_serve_fee" => $plateform_serve_fee, "store_final_fee" => $store_final_fee );
