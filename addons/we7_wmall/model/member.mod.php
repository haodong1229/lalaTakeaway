...
function member_credit_update($uid, $credittype, $creditval = 0, $log = array(  ), $wxtpl_notice = true)
{
    ...
    if( !empty($wxtpl_notice) ) 
    {
        ...
        {
            $params = array( "first" => "您在" . $config["mall"]["title"] . "的账户积分有新的变动", "keyword1" => date("Y-m-d H:i", TIMESTAMP), "keyword2" => (0 < $creditval ? "积分充值" : "积分消费"), "keyword3" => (string) $creditval . "积分", "remark" => implode("\n", array( "积分余额:" . $member["credit1"], "备注: " . $log[1] )) );
        }
        else
        {
            $params = array( "first" => "您在" . $config["mall"]["title"] . "的账户余额有新的变动", "keyword1" => date("Y-m-d H:i", TIMESTAMP), "keyword2" => (0 < $creditval ? "余额充值" : "余额消费"), "keyword3" => (string) $creditval . "元", "remark" => implode("\n", array( "账户余额:" . $member["credit2"], "备注: " . $log[1] )) );
        }
