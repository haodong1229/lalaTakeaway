...
function deliveryer_getcash_update($logOrId, $type, $extra = array())
{
	...
	if ($type == 'transfers') {
		...
		if (($log['channel'] == 'weixin') || ($log['channel'] == 'wxapp')) {
			...
			$params = array('partner_trade_no' => $log['trade_no'], 'openid' => ($log['channel'] == 'wxapp' ? $log['account']['openid_wxapp'] : $log['account']['openid']), 'check_name' => 'FORCE_CHECK', 're_user_name' => $log['account']['realname'], 'amount' => $log['final_fee'] * 100, 'desc' => $deliveryer['title'] . \date('Y-m-d H:i', $log['addtime']) . '提现申请');
			$response = $pay->mktTransfers($params);
		}
		 else if ($log['channel'] == 'alipay') {
			mload()->classs('alipay');
			$pay = new AliPay();
			$params = array('out_biz_no' => $log['trade_no'], 'payee_account' => $log['account']['account'], 'amount' => $log['final_fee'], 'payee_real_name' => $log['account']['realname'], 'remark' => $deliveryer['title'] . \date('Y-m-d H:i', $log['addtime']) . '提现申请');
			$response = $pay->transfer($params);
		}
		 else if ($log['channel'] == 'bank') {
			mload()->classs('wxpay');
			$pay = new WxPay();
			$params = array('partner_trade_no' => $log['trade_no'], 'enc_bank_no' => $log['account']['account'], 'enc_true_name' => $log['account']['realname'], 'bank_code' => $log['account']['id'], 'amount' => $log['final_fee'] * 100, 'desc' => $deliveryer['title'] . \date('Y-m-d H:i', $log['addtime']) . '提现申请');
...
function deliveryer_stat_order1($type = 'takeout')
{
	...
	if ($days == -1) {
		...
		$starttime = \str_replace('-', '', \trim($stat_day['start']));
		$endtime = \str_replace('-', '', \trim($stat_day['end']));
	...
	if ($type == 'takeout') {
		...
		$data = pdo_fetchall('select plateform_deliveryer_fee from ' . tablename('tiny_wmall_order') . $condition_takeout, $params);
...
function deliveryer_stat_detail($type = 'takeout')
{
	...
	if ($type == 'takeout') {
		$records_temp = pdo_fetchall('SELECT stat_day, count(*) as total_success_order, round(sum(plateform_deliveryer_fee), 2) as total_fee FROM ' . tablename('tiny_wmall_order') . $condition . ' and status = 5 and delivery_type = 2 group by stat_day', $params, 'stat_day');
	}
	 else if ($type == 'errander') {
	...
	while ($starttime <= $i) {
		if (empty($records_temp[$i])) {
			$records[] = array('stat_day' => $i, 'total_fee' => 0, 'total_success_order' => 0);
		}
		 else {
...
function deliveryer_stat_order()
{
	...
	$stat = array(
		'takeout'  => array('total_num' => 0, 'fee' => 0, 'num' => 0, 'cancel_num' => 0),
		'errander' => array('total_num' => 0, 'fee' => 0, 'num' => 0, 'cancel_num' => 0),
		...
		foreach ($deliveryers as &$row ) {
			...
			if ($row['work_status'] == 2) {
				...
				if ((($filter['order_type'] == 'is_takeout') || empty($filter['order_type'])) && ($row['work_status'] == 1) && empty($row['wait_pickup']) && empty($row['wait_delivery'])) {
				 ...
				 else if (($filter['order_type'] == 'is_errander') && empty($row['order_takeout_num']) && empty($row['order_errander_num'])) {
					$row['css'] = 'active';
				}
