...
function pindan_data_init1($sid, $pindan_id, $ignore_bargain = false)
{
    global $_W;
        ...
        foreach( $cart["data"] as $k => $v ) 
        {
            ...
            else
            {
                foreach( $v as $key => $val ) 
                {
                    ...
                    $cart["pindan"][$k][$key] = array( "cid" => $goods_info[$k]["cid"], "goods_id" => $k, "thumb" => tomedia($goods_info[$k]["thumb"]), "title" => $title, "option_title" => $option["name"], "num" => $num, "price" => $option["price"], "discount_price" => $option["price"], "discount_num" => 0, "price_num" => $num, "total_price" => round($option["price"] * $num, 2), "total_discount_price" => round($option["price"] * $num, 2), "bargain_id" => 0 );
                    $cart_goods[$k][$key] = $cart["pindan"][$k][$key];
                    $total_num += $num;
...
function order_fetch_pindan_data($id)
{
    ...
    $order = pdo_fetch("select a.id, a.sid, a.price, a.delivery_fee, a.pack_fee, a.discount_fee, a.extra_fee, a.final_fee, a.data, b.logo as store_logo, b.title as store_title from " . tablename("tiny_wmall_order") . " as a left join " . tablename("tiny_wmall_store") . " as b on a.sid = b.id " . $condition, $params);
    ...
    $avg_total = ($avg_delivery_fee + $avg_pack_fee + $avg_extra_fee) - $avg_discount_fee;
