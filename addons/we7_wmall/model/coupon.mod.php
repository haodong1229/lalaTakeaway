...
function coupon_before_timeout_notice()
{
    ...
    if( !empty($coupons) ) 
    {
        ...
        {
            $params = array( "first" => (string) $item["nickname"] . "，您的账户下有" . $item["num"] . "张优惠券即将过期，总价值" . $item["discount"] . "元，记得使用哦","keyword1" => date("Y-m-d H:i", TIMESTAMP), "keyword2" => "优惠券过期", "keyword3" => (string) $item["discount"] . "元", "remark" => implode("\n", array( "适用店铺: " . $config["mall"]["title"] . "合作商家", "使用规则: 限有效期内使用", "感谢您对" . $config["mall"]["title"] . "平台的支持与厚爱。点击查看详情>>" )) );
