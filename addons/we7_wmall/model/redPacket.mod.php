...
function redPacket_grant($params, $wxtpl_notice = true)
{
	...
	if (!empty($wxtpl_notice)) {
		...
		$params = array('first' => '您在' . $config['mall']['title'] . '的账户有新的红包', 'keyword1' => date('Y-m-d H:i', TIMESTAMP), 'keyword2' => '红包到账', 'keyword3' => $params['discount'] . '元', 'remark' => implode('
...
{
	$channel = array(
		''                   => array('text' => '未知', 'css' => 'label-danger'),
		'shareRedpacket'     => array('text' => '分享红包', 'css' => 'label-success'),
		'freeLunch'          => array('text' => '霸王餐', 'css' => 'label-info'),
		'superRedpacket'     => array('text' => '超级红包', 'css' => 'label-warning'),
		'mealRedpacket'      => array('text' => '套餐红包', 'css' => 'label-warning'),
		'mealRedpacket_plus' => array('text' => '套餐红包plus', 'css' => 'label-warning')
		);
	return $channel;
}
...
{
	$status = array(
		1 => array('text' => '未使用', 'css' => 'label-info'),
		2 => array('text' => '已使用', 'css' => 'label-success'),
		3 => array('text' => '已过期', 'css' => 'label-default')
		);
	return $status;
}
...
function redpacket_before_timeout_notice()
{
	...
	if (!empty($redpackets)) {
		...
		foreach ($data as $item) {
			$params = array('first' => $item['nickname'] . '，您的账户下有' . $item['num'] . '红包即将过期，总价值' . $item['discount'] . '元，记得使用哦~', 'keyword1' => date('Y-m-d H:i', TIMESTAMP), 'keyword2' => '红包即将过期', 'keyword3' => $item['discount'] . '元', 'remark' => implode('
