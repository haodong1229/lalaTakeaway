        ...
        else
        {
            if( $ta == "fee" ) 
            {
                ...
                if( $_W["ispost"] ) 
                {
                    ...
                    $update = array( "fee_takeout" => iserializer($fee_takeout), "fee_selfDelivery" => iserializer($fee_selfDelivery), "fee_instore" => iserializer($fee_instore), "fee_paybill" => iserializer($fee_paybill), "fee_limit" => intval($account["fee_limit"]), "fee_rate" => floatval($account["fee_rate"]), "fee_min" => floatval($account["fee_min"]), "fee_max" => floatval($account["fee_max"]), "fee_period" => intval($account["fee_period"]) );
