...
if($ta == 'setting') {
	...
	if($_W['ispost']) {
		...
		if(!empty($type)) {
			...
			} elseif($type == 'thumbs' || $type == 'qualification') {
...
elseif($ta == 'account') {
	...
	if($_W['ispost']) {
		...
		$update = array(
			'delivery_type' => intval($store['delivery_type']),
			'delivery_within_days' => intval($store['delivery_within_days']),
			'delivery_reserve_days' => intval($store['delivery_reserve_days']),
			'serve_radius' => floatval($store['serve_radius']),
			'delivery_area' => trim($store['delivery_area']),
			'not_in_serve_radius' => intval($store['not_in_serve_radius']),
			'auto_get_address' => intval($store['auto_get_address']),
			'delivery_time' => intval($store['delivery_time']),
			'pack_price' => floatval($store['pack_price']),
			'delivery_fee_mode' => intval($store['delivery_fee_mode']),
			'send_price' => floatval($store['send_price']),
			'delivery_free_price' => floatval($store['delivery_free_price']),
		);
		$delivery_extra = array(
			'store_bear_deliveryprice' => floatval($store['delivery_extra']['store_bear_deliveryprice']),
			'plateform_bear_deliveryprice' => floatval($store['delivery_extra']['plateform_bear_deliveryprice']),
		...
		if($update['delivery_fee_mode'] == 1) {
			...
			$delivery_price = array(
				'start_fee' => floatval($store['delivery_price_extra']['start_fee']),
				'start_km' => floatval($store['delivery_price_extra']['start_km']),
				'pre_km_fee' => floatval($store['delivery_price_extra']['pre_km_fee']),
				'over_km' => floatval($store['delivery_price_extra']['over_km']),
				'over_pre_km_fee' => floatval($store['delivery_price_extra']['over_pre_km_fee']),
				'max_fee' => floatval($store['delivery_price_extra']['max_fee']),
				'calculate_distance_type' => intval($store['delivery_price_extra']['calculate_distance_type']),
				'distance_type' => intval($store['delivery_price_extra']['distance_type'])
			);
...
elseif($ta == 'pill') {
	...
	if($_W['ispost']) {
		$store = json_decode(htmlspecialchars_decode($_GPC['store']), true);
		$update = array(
			'auto_handel_order' => intval($store['auto_handel_order']),
			'auto_notice_deliveryer' => intval($store['auto_notice_deliveryer']),
			'invoice_status' => intval($store['invoice_status']),
			'token_status' => intval($store['token_status']),
			'payment' => iserializer(array_map('trim', $store['payment'])),
		);
		pdo_update('tiny_wmall_store', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
...
elseif($ta == 'fee') {
	...
	if($_W['ispost']) {
		...
		} elseif($fee_selfDelivery['type'] == 2) {
		...
		pdo_update('tiny_wmall_store_account', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));

		$store = json_decode(htmlspecialchars_decode($_GPC['store']), true);
		$data = array(
			'status' => intval($store['data']['superCoupon']['status']),
			'max_limit' => intval($store['data']['superCoupon']['max_limit'])
		);
