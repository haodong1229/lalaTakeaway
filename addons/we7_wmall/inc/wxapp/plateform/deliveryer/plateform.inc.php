...
elseif($ta == 'post') {
	...
	if($_W['ispost']) {
		...
		$data = array(
			'uniacid' => $_W['uniacid'],
			'mobile' => $mobile,
			'title' => trim($deliveryer['title']),
			'openid' => trim($deliveryer['openid']),
			'openid_wxapp' => trim($deliveryer['openid_wxapp']),
			'nickname' => trim($deliveryer['nickname']),
			'avatar' => trim($deliveryer['avatar']),
			'sex' => trim($deliveryer['sex']),
			'age' => intval($deliveryer['age']),
		);
	...
	if($_W['ispost']) {
		...
		if($deliveryer_takeout_fee_type == 1 || $deliveryer_takeout_fee_type == 2) {
			...
			$deliveryer_takeout_fee = array(
				'start_fee' => floatval($fee_deliveryer_takeout['deliveryer_fee']['start_fee']),
				'start_km' => floatval($fee_deliveryer_takeout['deliveryer_fee']['start_km']),
				'pre_km' => floatval($fee_deliveryer_takeout['deliveryer_fee']['pre_km']),
				'max_fee' => floatval($fee_deliveryer_takeout['deliveryer_fee']['max_fee'])
			);
		}
		...
		if($deliveryer_errander_fee_type == 1 || $deliveryer_errander_fee_type == 2) {
			...
			$deliveryer_errander_fee = array(
				'start_fee' => floatval($fee_deliveryer_errander['deliveryer_fee']['start_fee']),
				'start_km' => floatval($fee_deliveryer_errander['deliveryer_fee']['start_km']),
				'pre_km' => floatval($fee_deliveryer_errander['deliveryer_fee']['pre_km']),
				'max_fee' => floatval($fee_deliveryer_errander['deliveryer_fee']['max_fee'])
			);
		}
		...
		$update = array(
			'fee_delivery' => iserializer($fee_delivery),
		);
		if(empty($_W['agentid'])) {
			$fee_getcash = array(
				'get_cash_fee_limit' => intval($deliveryer['fee_getcash']['get_cash_fee_limit']),
				'get_cash_fee_min' => floatval($deliveryer['fee_getcash']['get_cash_fee_min']),
				'get_cash_fee_max' => floatval($deliveryer['fee_getcash']['get_cash_fee_max']),
				'get_cash_fee_rate' => floatval($deliveryer['fee_getcash']['get_cash_fee_rate']),
				'get_cash_period' => intval($deliveryer['fee_getcash']['get_cash_period'])
			);
...
elseif($ta == 'power') {
	...
	if($_W['ispost']) {
		$deliveryer = $_GPC['deliveryer'];
		$perm_cancel = array(
			'status_takeout' => intval($deliveryer['perm_cancel']['status_takeout']),
			'status_errander' => intval($deliveryer['perm_cancel']['status_errander'])
		);
		$perm_transfer = array(
			'status_takeout' => intval($deliveryer['perm_transfer']['status_takeout']),
			'status_errander' => intval($deliveryer['perm_transfer']['status_errander']),
			'max_takeout' => intval($deliveryer['perm_transfer']['max_takeout']),
			'max_errander' => intval($deliveryer['perm_transfer']['max_errander']),
		);
		$update = array(
			'is_takeout' => intval($deliveryer['is_takeout']),
			'is_errander' => intval($deliveryer['is_errander']),
			'perm_cancel' => iserializer($perm_cancel),
			'perm_transfer' => iserializer($perm_transfer),
			'collect_max_takeout' => intval($deliveryer['collect_max_takeout']),
			'collect_max_errander' => intval($deliveryer['collect_max_errander']),
		);
		pdo_update('tiny_wmall_deliveryer', $update, array('uniacid' => $_W['uniacid'], 'id' => $id));
