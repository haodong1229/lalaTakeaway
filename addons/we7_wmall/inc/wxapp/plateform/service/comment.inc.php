...
if( $ta == "list" ) 
{
    ...
    $comments = pdo_fetchall("SELECT a.*, b.uid,b.openid,b.addtime FROM " . tablename("tiny_wmall_order_comment") . " AS a LEFT JOIN " . tablename("tiny_wmall_order") . " AS b ON a.oid = b.id " . $condition . " ORDER BY a.addtime DESC LIMIT " . ($page - 1) * $psize . "," . $psize, $params);
    if( !empty($comments) ) 
    {
        $stores = store_fetchall(array( "title" ));
