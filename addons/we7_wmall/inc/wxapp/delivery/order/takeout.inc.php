...
if( $ta == "list" ) 
{
    ...
    $orders = pdo_fetchall("SELECT id,order_plateform,serial_sn, addtime, is_pay, pay_type,order_type, status, username, mobile, address,distance, delivery_status,deliveryer_id,plateform_deliveryer_fee, delivery_type, delivery_fee, delivery_day, delivery_time,delivery_assign_time, sid, num, final_fee, delivery_collect_type, transfer_delivery_status,location_x,location_y,note FROM " . tablename("tiny_wmall_order") . $condition . $order_by . $limit, $params);
    if( !empty($orders) ) 
    {
        $stores_id = array(  );
