...
if ($ta == 'index') {
	...
	$result = array('store' => $store, 'coupon' => $coupons, 'cart' => cart_data_init($sid), 'template' => $template, 'template_page' => $template_page, 'lazyload_goods' => $_config_mall['lazyload_goods']);
...
if ($ta == 'pindan') {
	...
	if (empty($pindan_id)) {
		...
		if (empty($cart)) {
			...
			if ($cart['pindan_id'] == 0) {
			 ...
			 else if ($cart['pindan_id'] != $cart['id']) {
				$is_founder = 0;
				$pindan_id = $cart['pindan_id'];
			}
	 ...
	 else if (empty($cart['pindan_id'])) {
		$not_takepart = 1;
	}
	 else if ((0 < $cart['pindan_id']) && ($cart['pindan_id'] != $pindan_id)) {
	...
	$result = array(
		'pindan'            => $pindan,
		'store'             => $store,
		'extra'             => array('not_takepart' => $not_takepart, 'is_founder' => $is_founder),
		'cartSendCondition' => $store['send_price'] - $pindan['total_cart_price'],
		'sharedata'         => array('title' => '来一起点外卖吧', 'desc' => '我想点' . $store['title'] . '，快来一起吧', 'imageUrl' => tomedia($store['logo']), 'path' => '/pages/store/pindan?sid=' . $sid . '&pindan_id=' . $pindan_id)
		);
	imessage(error(0, $result), '', 'ajax');
	return 1;
}
...
if ($ta == 'giveupPindan') {
	...
	$result = array(
		'pindan'            => $pindan,
		'cartSendCondition' => $store['send_price'] - $pindan['total_cart_price'],
		'extra'             => array('not_takepart' => $not_takepart)
		);
	imessage(error(0, $result), '', 'ajax');
	return 1;
}
...
if ($ta == 'takePartPindan') {
	...
	if (empty($cart)) {
	 ...
	 else if ($cart['pindan_id'] != $pindan_id) {
		if (0 < $cart['pindan_id']) {
...
if ($ta == 'search') {
	...
	if (!($is_search)) {
		$result = array('hotGoods' => pdo_getall('tiny_wmall_goods', array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'status' => 1, 'is_hot' => 1), array('id', 'title')), 'store' => $store);
		imessage(error(0, $result), '', 'ajax');
	}
	...
	$goods = goods_filter($sid);
	$result = array('store' => $store, 'goods' => $goods);
