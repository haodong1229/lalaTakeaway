...
if( $ta == "index" ) 
{
    ...
    $tables = pdo_fetchall("select count(*) as total_num, cid from " . tablename("tiny_wmall_tables") . " where uniacid = :uniacid and sid = :sid group by cid", array( ":uniacid" => $_W["uniacid"], ":sid" => $sid ), "cid");
...
else
{
    if( $ta == "post" ) 
    {
        ...
        $tables = table_fetchall(array( "sid" => $sid, "cid" => $cid ));
        if( empty($tables) ) 
        {
        ...
        $condition = " where uniacid = :uniacid and sid = :sid and order_type = :order_type and stat_day > :stat_day and status < :status and table_cid = :table_cid and reserve_time = :reserve_time";
        ...
        $reserve_orders = pdo_fetchall("select id, table_id from " . tablename("tiny_wmall_order") . $condition, $params, "table_id");
        if( !empty($reserve_orders) ) 
        {
            $table_num = count($tables);
            $reserved_num = count($reserve_orders);
            if( $table_num <= $reserved_num ) 
            {
                imessage(error(-1, "该桌台类型下没有可以预订的桌位"), "", "ajax");
            }
