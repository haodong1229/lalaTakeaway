...
else
{
    if( $ta == "stick" ) 
    {
        ...
        if( $_W["ispost"] ) 
        {
            ...
            $update = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "type" => $type, "displayorder" => $displayorder, "title" => $title, "status" => 0, "addtime" => TIMESTAMP, "starttime" => TIMESTAMP, "endtime" => TIMESTAMP, "is_pay" => 0, "order_sn" => date("YmdHis", time()) . random(6, true), "final_fee" => $finalfee, "pay_type" => $pay_type, "days" => $day, "data" => "" );
            if( $type == "stick" ) 
            {
    ...
    else
    {
        if( $ta == "recommend" ) 
        {
            ...
            if( $_W["ispost"] ) 
            {
                ...
                $update = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "type" => $type, "title" => (string) $page_title . "幻灯片展示" . $day . "天", "status" => 0, "addtime" => TIMESTAMP, "starttime" => TIMESTAMP, "endtime" => TIMESTAMP, "is_pay" => 0, "order_sn" => date("YmdHis", time()) . random(6, true), "final_fee" => $finalfee, "pay_type" => $pay_type, "days" => $day, "data" => "" );
                pdo_insert("tiny_wmall_advertise_trade", $update);
                $id = pdo_insertid();
                imessage(error(0, array( "id" => $id, "sid" => $sid )), "", "ajax");
            }
