        ...
        else
        {
            if( $ta == "activity_svipRedpacket" ) 
            {
                ...
                if( $_W["ispost"] ) 
                {
                    ...
                    mload()->model("plugin");
                    pload()->model("svip");
                    $redpacket = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => $store["title"], "discount" => $discount, "condition" => $condition, "use_days_limit" => $use_days_limit, "amount" => $amount, "starttime" => $starttime, "endtime" => $endtime, "data" => array( "plateform_charge" => 0, "agent_charge" => 0, "store_charge" => $discount ) );
