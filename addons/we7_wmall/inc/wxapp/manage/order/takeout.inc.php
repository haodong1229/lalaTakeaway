...
if($ta == 'list') {
	...
	if(!empty($orders)) {
		...
		foreach($orders as &$da) {
			...
			if($da['status'] >= 2 && $config['customer_cancel_status'] == 1 && $config['customer_cancel_timelimit'] > 0) {
	...
	$result = array(
		'orders' => $orders,
		'openid_wxapp_manager' => $_W['manager']['openid_wxapp_manager'],
		'store' => array(
			'data' => $_W['we7_wmall']['store']['data'],
			'cn' => $_W['we7_wmall']['store']['cn']
		),
	);
...
elseif($ta == 'push_uupaotui') {
	...
	if ($_W['ispost']) {
		...
		if(!$push) {
			$result = array(
				'tips' => "UU跑腿需要配送费{$status['need_paymoney']}元，确定推送到UU跑腿吗？",
				'id' => $id
			);
		} else {
			$result = '订单推送到UU跑腿成功';
...
elseif($ta == 'handleGoodsRefund') {
	...
	if($type == 'agree') {
		...
		$update_order = array(
			'store_final_fee' => $order['store_final_fee'] - $calculate['refund_total_fee'],
			'data' => iserializer($data),
			'refund_fee' => $order['refund_fee'] + $calculate['refund_total_fee'],
		);
...
elseif($ta == 'yinsihao') {
	...
	$data = yinsihao_bind($order_id, 'member');
	if(is_error($data)) {
