...
if ($ta == 'info') {
	...
	if (empty($type)) {
	 ...
	 else if ($type == 'haodian_category') {
		$filter = array('haodian_cid', 'haodian_child_id');
	}
	 else if ($type == 'haodian_tags') {
...
if ($ta == 'pill') {
	...
	$store = store_fetch($sid, array('auto_handel_order', 'auto_notice_deliveryer', 'invoice_status', 'token_status', 'payment'));
