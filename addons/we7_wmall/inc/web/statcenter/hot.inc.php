...
if( $op == "index" ) 
{
    ...
    $chart = array( "field" => array( "goods_num", "goods_price" ), "title" => array(  ), "data" => array(  ) );
    if( !empty($records) ) 
    {
        ...
        foreach( $records as &$row ) 
        {
            ...
            $chart["data"]["goods_price"][] = array( "name" => $row["goods_title"], "value" => $row["total_goods_price"] );
            $chart["data"]["goods_num"][] = array( "name" => $row["goods_title"], "value" => $row["total_goods_num"] );
        }
    }
    ...
    if( $_GPC["r"] == "export" ) 
    {
        $stat_fields = array( "goods_title" => array( "field" => "goods_title", "title" => "商品名称", "width" => "30" ), "store_name" => array( "field" => "store_name", "title" => "店铺名称", "width" => "20" ), "goods_number" => array( "field" => "goods_number", "title" => "商品编号", "width" => "20" ), "option" => array( "field" => "option", "title" => "规格", "width" => "20" ), "total_goods_num" => array( "field" => "total_goods_num", "title" => "销售量", "width" => "30" ), "total_goods_price" => array( "field" => "total_goods_price", "title" => "销售额", "width" => "20" ), "pre_goods_price" => array( "field" => "pre_goods_price", "title" => "销售额百分比", "width" => "20" ), "pre_goods_num" => array( "field" => "pre_goods_num", "title" => "销售量百分比", "width" => "20" ) );
        ...
        if( !empty($records) ) 
        {
            ...
            {
                $val["total_goods_num"] = (string) $val["total_goods_num"] . $val["goods_unitname"];
                foreach( $all_fields as $key => $li ) 
                {
                ...
                if( !empty($val["options"]) ) 
                {
                    ...
                    {
                        $temp = array( "goods_title" => "", "goods_number" => "", "option" => $option["name"], "total_goods_num" => (string) $option["option_goods_num"] . $val["goods_unitname"] );
                        foreach( $all_fields as $key => $li ) 
                        {
