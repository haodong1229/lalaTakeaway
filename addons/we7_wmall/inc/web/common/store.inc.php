...
if( $op == "list" ) 
{
    ...
    {
        $key = trim($_GPC["key"]);
        $data = pdo_fetchall("select id, title, logo, delivery_free_price, content, score,sailed,send_price,delivery_price,delivery_time, `data` from " . tablename("tiny_wmall_store") . " where uniacid = :uniacid and is_waimai = 1 and title like :key order by id desc limit 50", array( ":uniacid" => $_W["uniacid"], ":key" => "%" . $key . "%" ), "id");
        if( !empty($data) ) 
        {
            foreach( $data as &$row ) 
            {
