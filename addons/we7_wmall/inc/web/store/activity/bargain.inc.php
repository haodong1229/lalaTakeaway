...
if( $ta == "list" ) 
{
    ...
    $psize = 20;
    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("tiny_wmall_activity_bargain") . " where uniacid = :uniacid and agentid = :agentid and  sid = :sid", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":sid" => $store["id"] ));
    $bargains = pdo_fetchall("select * from" . tablename("tiny_wmall_activity_bargain") . " where uniacid = :uniacid and agentid = :agentid and sid = :sid order by id desc limit " . ($pindex - 1) * $psize . "," . $psize, array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":sid" => $store["id"] ), "id");
