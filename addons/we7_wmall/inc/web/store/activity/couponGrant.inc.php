...
if( $ta == "post" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $activity = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => $title, "starttime" => $starttime, "endtime" => $endtime, "type" => "couponGrant", "status" => 1, "data" => iserializer($_GPC["coupons"]) );
        ...
        $coupon = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "activity_id" => $status, "condition" => trim($_GPC["condition"]), "title" => trim($_GPC["title"]), "starttime" => $starttime, "endtime" => $endtime, "type" => "couponGrant", "status" => 1, "amount" => intval($_GPC["amount"]), "coupons" => iserializer($_GPC["coupons"]) );
