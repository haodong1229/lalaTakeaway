...
if( $ta == "index" ) 
{
	...
	if( $_W["ispost"] ) 
	{
		...
		mload()->model("plugin");
		pload()->model("svip");
		$redpacket = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => $store["title"], "discount" => $discount, "condition" => $condition, "use_days_limit" => $use_days_limit, "amount" => $amount, "starttime" => $starttime, "endtime" => $endtime, "data" => array( "discount_bear" => array( "plateform_charge" => $plateform_charge, "agent_charge" => $agent_charge, "store_charge" => $store_charge ) ) );
