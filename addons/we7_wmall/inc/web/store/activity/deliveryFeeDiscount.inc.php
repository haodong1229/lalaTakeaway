...
if( $ta == "index" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $title = implode(",", $title);
        $activity = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => $title, "starttime" => $starttime, "endtime" => $endtime, "type" => "deliveryFeeDiscount", "status" => 1, "data" => iserializer($data) );
        ...
        imessage(error(0, "设置满减配送费活动成功"), "refresh", "ajax");
    }
...
if( $ta == "del" ) 
{
    $status = activity_del($sid, "deliveryFeeDiscount");
    if( is_error($status) ) 
    {
...
include(itemplate("store/activity/deliveryFeeDiscount"));
