...
if( $ta == "index" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $activity = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "title" => "平台新用户立减" . $back . "元", "starttime" => $starttime, "endtime" => $endtime, "type" => "mallNewMember", "status" => 1, "data" => array( "back" => $back, "plateform_charge" => 0, "store_charge" => $back ) );
        if( !empty($_W["ismanager"]) ) 
        {
            ...
            else
            {
                ...
                {
                    if( $back < $activity["data"]["plateform_charge"] + $activity["data"]["agent_charge"] ) 
                    {
