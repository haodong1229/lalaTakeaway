...
if( $ta == "post" ) 
{
    ...
    if( !empty($id) ) 
    {
        ...
        if( empty($item) ) 
        {
            imessage("商品不存在或已删除", iurl("store/seckill/goods_list/list"), "info");
        }
    ...
    if( $_W["ispost"] ) 
    {
        ...
        $data = array( "uniacid" => $_W["uniacid"], "sid" => $sid, "cid" => intval($_GPC["cid"]), "title" => trim($_GPC["title"]), "price" => floatval($_GPC["price"]), "old_price" => floatval($_GPC["old_price"]), "total" => intval($_GPC["total"]), "thumb" => trim($_GPC["thumb"]), "displayorder" => intval($_GPC["displayorder"]), "content" => trim($_GPC["content"]), "description" => trim($_GPC["description"]), "starttime" => intval($starttime), "endtime" => intval($endtime), "use_limit_day" => intval($_GPC["use_limit_day"]) );
        $data["slides"] = array(  );
        ...
        imessage(error(0, "编辑商品成功"), iurl("store/seckill/goods_list/list"), "ajax");
    }
...
if( $ta == "list" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        ...
        imessage(error(0, "修改成功"), iurl("store/seckill/goods_list/list"), "ajax");
    }
...
include(itemplate("store/seckill/goods_list"));
