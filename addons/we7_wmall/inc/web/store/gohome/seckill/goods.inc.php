...
if( $ta == "list" ) 
{
    ...
    if( $_W["ispost"] ) 
    {
        if( !empty($_GPC["ids"]) ) 
        {
            ...
            {
                $data = array( "name" => trim($_GPC["name"][$k]), "price" => floatval($_GPC["price"][$k]), "old_price" => floatval($_GPC["old_price"][$k]), "total" => intval($_GPC["total"][$k]), "use_limit_day" => intval($_GPC["use_limit_day"][$k]) );
