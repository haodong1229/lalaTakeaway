...
if( $ta == "post" ) 
{
    ...
    if( $_W["isajax"] ) 
    {
        $data = array( "sid" => $sid, "uniacid" => $_W["uniacid"], "title" => trim($_GPC["title"]), "price" => floatval($_GPC["price"]), "ts_price" => floatval($_GPC["ts_price"]), "box_price" => floatval($_GPC["box_price"]), "unitname" => trim($_GPC["unitname"]), "total" => intval($_GPC["total"]), "sailed" => intval($_GPC["sailed"]), "status" => intval($_GPC["status"]), "cid" => intval($_GPC["cid"]), "type" => intval($_GPC["type"]), "thumb" => trim($_GPC["thumb"]), "label" => trim($_GPC["label"]), "displayorder" => intval($_GPC["displayorder"]), "description" => htmlspecialchars_decode($_GPC["description"]), "is_hot" => intval($_GPC["is_hot"]) );
