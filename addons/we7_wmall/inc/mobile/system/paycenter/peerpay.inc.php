...
if( $ta == "message" ) 
{
    ...
    if( empty($peerpay) ) 
    {
        $peerpay = array( "uniacid" => $_W["uniacid"], "uid" => $paylog["uid"], "plid" => $paylog["id"], "orderid" => $paylog["order_id"], "peerpay_type" => $type, "peerpay_price" => $paylog["fee"], "peerpay_realprice" => $paylog["fee"], "createtime" => TIMESTAMP, "data" => $paylog["data"] );
...
if( $ta == "paylist" ) 
{
    ...
    $member = get_member($paylog["uid"]);
    $_W["page"]["title"] = (string) $member["nickname"] . "的代付";
...
$_share = array( "title" => $title, "desc" => $peerpay["peerpay_message"], "imgUrl" => tomedia($peerpay["data"]["logo"]), "link" => imurl("system/paycenter/peerpay/paylist", array( "id" => $peerpay["id"] ), true) );
