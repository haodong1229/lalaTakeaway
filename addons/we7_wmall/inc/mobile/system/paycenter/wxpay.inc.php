...
if( $_GPC["done"] == "1" ) 
{
    ...
    if( !empty($log) ) 
    {
        ...
        {
            $record = pdo_get("tiny_wmall_paylog", array( "uniacid" => $log["uniacid"], "order_sn" => $log["tid"] ));
            imessage("顾客已支付成功,服务器没有接受到微信官方的支付成功推送。这个问题不是模块的问题,可能是环境配置问题。单号：" . $record["order_id"] . ",类型：" . $record["order_type"], "", "error");
        }
