...
<ul class="we7-page-tab">
	<?php  if(!empty($module['isrulefields']) &&!empty($_W['account']) && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) { ?><li><a href="<?php  echo url('platform/reply', array('m' => $module['name'], 'version_id' => intval($_GPC['version_id'])));?>">关键字链接入口 </a></li><?php  } ?>
	<?php  if(!empty($frames['section']['platform_module_common']['menu']['platform_module_cover'])) { ?>
	<li class="active"><a href="<?php  echo url('platform/cover', array('m' => $module['name'], 'eid' => $entry_id, 'version_id' => intval($_GPC['version_id'])));?>">封面链接入口</a></li>
	<?php  } ?>
</ul>
