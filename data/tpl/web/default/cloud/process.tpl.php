			...
			if(!path && i >= total) {
				if ($scope.fails && $scope.fails.length > 0) {
					...
					if (config.type == 'theme') {
						...
						targetUrl = './index.php?c=cloud&a=process&step=schemas&m=' + config.appname + '&is_upgrade=' + config.is_upgrade + '&batch=1&support=' + config.account_type;
					}
...
<div class="clearfix js-processor">
	<?php  if(empty($packet['schemas'])) { ?>
		...
		<script>
			<?php  if($_GPC['batch']) { ?>
				location.href = '<?php  echo url("cloud/process", array("step" => "scripts", "m" => $m, "is_upgrade" => $is_upgrade, "batch" => 1, "support" => $_GPC["support"]));?>';
			<?php  } else { ?>
				location.href = '<?php  echo url("cloud/process", array("step" => "scripts", "m" => $m, "is_upgrade" => $is_upgrade, "support" => $_GPC["support"]));?>';
			<?php  } ?>
		</script>
		<?php  } ?>
